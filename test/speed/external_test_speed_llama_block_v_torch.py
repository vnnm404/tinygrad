...
class TestBlockLayers(unittest.TestCase):
  """Test individual components within a transformer block."""
  ...
  def setUpClass(cls):
    ...
    # KV cache (simulating mid-generation state)
    cls.start_pos = 64
  ...
  def test_00_embedding_lookup(self):
    """Embedding lookup from vocab to DIM."""
    vocab_size = 10000
  ...
  def test_02_wq_projection(self):
    """Query projection: (1, 1, 2048) @ (2048, 2048) -> (1, 1, 2048)"""
  ...
  def test_03_wk_projection(self):
    """Key projection: (1, 1, 2048) @ (2048, 512) -> (1, 1, 512)"""
  ...
  def test_04_wv_projection(self):
    """Value projection: (1, 1, 2048) @ (2048, 512) -> (1, 1, 512)"""
  ...
  def test_07_repeat_kv(self):
    ...
    # use full cache shape after repeat for testing
    torch.manual_seed(0)
  ...
  def test_09_wo_projection(self):
    """Output projection: (1, 1, 2048) @ (2048, 2048) -> (1, 1, 2048)"""
  ...
  def test_12_ffn_w1(self):
    """FFN gate projection: (1, 1, 2048) @ (2048, 8192) -> (1, 1, 8192)"""
  ...
  def test_13_ffn_w3(self):
    """FFN up projection: (1, 1, 2048) @ (2048, 8192) -> (1, 1, 8192)"""
  ...
  def test_15_ffn_gate_mul(self):
    """Element-wise multiply of silu(w1) * w3."""
    torch.manual_seed(0)
  ...
  def test_16_ffn_w2(self):
    """FFN down projection: (1, 1, 8192) @ (8192, 2048) -> (1, 1, 2048)"""
    torch.manual_seed(0)
    torch_h = (torch.rand(1, SEQ_LEN, HIDDEN_DIM, dtype=torch_dt) - 0.5).to(torch_device)
    tiny_h = Tensor(torch_h.cpu().numpy())
  ...
  def test_18_output_projection(self):
    """Output projection to vocab: (1, 1, 2048) @ (2048, 128256) -> (1, 1, 128256)"""
